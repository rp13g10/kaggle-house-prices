...
regressors = [
    ...
    linear_model.Ridge(**k),
    linear_model.Lasso(**k),
    linear_model.ElasticNet(**k),
    linear_model.BayesianRidge(),
    linear_model.LogisticRegression(**k),
    linear_model.SGDRegressor(**k),
...
for candidate in cand_iter:
    ...
    pred_df = pd.DataFrame(val.index, columns=['Id'])
...
#%% Take a look at the updated results
results

#%% See if sticking all of the above models together improves performance

parliament = ensemble.VotingRegressor(
    estimators=models)

parliament.fit(X_train, y_train)
